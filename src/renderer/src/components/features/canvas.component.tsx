...
function ImageCanvas() {
  ...
  const { texts, selectedTextId, updateTextElement, deleteTextElement } = useTextState()

  const { standaloneDots, selectedDotId, updateDotElement, deleteDotElement } = useDotState()
  ...
  const dots = useMemo(() => {
    ...
    return result
  }, [JSON.stringify(points), area, amount, size, gap, padding, rotation, randomJitter])
  ...
  return (
    <main className="flex h-full w-full border rounded" onContextMenu={(e) => e.preventDefault()}>
      <TransformWrapper
        limitToBounds={false}
        initialScale={1}
        minScale={0.5}
