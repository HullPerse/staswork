...
import { DotElement, Points, PointsHistory, TextElement } from '@renderer/types'
...
import { useDotDrag, useTextDrag } from '@renderer/hook/canvas.hook'
...
export default function ModeRenderer({
  ...
  const { texts, selectedTextId, setTexts, textSettings, setSelectedTextId, updateTextElement } =
    useTextState()
  const {
    standaloneDots,
    selectedDotId,
  ...
  const { setArea, points, setPoints, size, setResults, editIndex, textMode, dotMode } =
    useCanvasState()
  ...
  const handleCanvasClick = useCallback(
    (e: MouseEvent<SVGSVGElement>) => {
      ...
      if (target.tagName === 'text' || target.tagName === 'g' || target.tagName === 'circle') {
        return
      }
