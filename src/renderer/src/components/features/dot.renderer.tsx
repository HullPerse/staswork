import { DotElement } from '@renderer/types'
...
function GeneratedDots({ dots, size, fill = 'black' }: GeneratedDotsProps) {
  return (
    <>
      {dots.map((dot, index) => (
        <circle key={`generated-${index}`} cx={dot.cx} cy={dot.cy} r={size / 2} fill={fill} />
      ))}
    </>
  )
}
...
function HistoryDots({ dots, size, fill = 'black' }: HistoryDotsProps) {
  return (
    <>
      {dots.map((dot, index) => (
        <circle key={`history-dot-${index}`} cx={dot.cx} cy={dot.cy} r={size / 2} fill={fill} />
      ))}
    </>
  )
}
