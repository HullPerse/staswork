...
const ImageComponent = ({ src, alt, className, width, height, ...props }: ImageProps) => {
  const [isLoaded, setIsLoaded] = useState(false)
  const [hasError, setHasError] = useState(false)

  const imgRef = useRef<HTMLImageElement>(null)

  const isExternal = src.startsWith('http') || src.startsWith('blob:') || src.startsWith('data:')
  ...
  return (
    <div
      className={cn('relative overflow-hidden items-center flex w-full', className)}
      style={{
        aspectRatio: width && height ? `${width}/${height}` : undefined
      }}
    >
