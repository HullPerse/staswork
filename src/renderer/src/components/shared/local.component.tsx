import { calculatePercentage, calculateProportions, getPolygonArea } from '@renderer/lib/utils'
import { Points } from '@renderer/types'
import { Edit, EllipsisVertical, Eye, EyeOff, Grip, Percent, Trash } from 'lucide-react'
...
export default function localHistory() {
  ...
  const getPercentage = useCallback(
    (points: Points[]) => {
      const allAreas = history.map((item) => getPolygonArea(item.settings.points))
  ...
  return history.map((item, index) => (
    <section key={index} className="flex flex-row w-full h-14 min-h-14 border rounded">
      <span className="font-bold border-r h-full items-center flex p-1">Область {index + 1}</span>
      ...
      <div className="flex flex-row ml-auto p-2 h-full items-center justify-center gap-2">
        ...
        <button className="cursor-pointer" onClick={() => toggleHistoryVisibility(index)}>
          {item.visible ? <Eye /> : <EyeOff />}
        </button>
        <button
          className="cursor-pointer"
          onClick={() => {
            updateActiveImageHistory([...history.slice(0, index), ...history.slice(index + 1)])
          }}
        >
          <Trash />
        </button>
