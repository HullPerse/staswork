...
import { createImageDataWithMetadata } from '@renderer/lib/utils'
import ImageStorage from '@renderer/service/image.service'
import type { ProcessedFile } from '@renderer/service/image.service'

const CanvasStateContext = createContext<TCanvasStateContext | undefined>(undefined)
...
export function CanvasProvider({ children }: { children: ReactNode }) {
  ...
  const [results, setResults] = useState<CanvasState['results']>([{ cx: 0, cy: 0 }])
  ...
  const [randomJitter, setRandomJitter] = useState<CanvasState['randomJitter']>(false)
    ...
    async (files: File[]) => {
      ...
      try {
        ...
        const newImageData = await Promise.all(
          allProcessedFiles.map((processedFile) => createImageDataWithMetadata(processedFile))
        )
  ...
  const updateActiveImageHistory = useCallback(
    (newHistory: PointsHistory[]) => {
      ...
      setImageHistory((prev) =>
        prev.map((img) => (img.id === activeImageId ? { ...img, editHistory: newHistory } : img))
      )
    },
    [activeImageId]
  )
  ...
  const updateActiveImageTexts = useCallback(
    (newTexts: TextElement[]) => {
      ...
      setImageHistory((prev) =>
        prev.map((img) => (img.id === activeImageId ? { ...img, currentTexts: newTexts } : img))
      )
    },
    [activeImageId]
  )
  ...
  const updateActiveImageDots = useCallback(
    (newDots: DotElement[]) => {
      ...
      setImageHistory((prev) =>
        prev.map((img) =>
          img.id === activeImageId ? { ...img, currentStandaloneDots: newDots } : img
        )
      )
    },
    [activeImageId]
