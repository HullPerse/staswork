import { calculateProportions, getPolygonArea } from "@/lib/utils";
...
export default function globalHistory() {
  const { imageHistory, activeImageId, amount, handleImageSelect } =
    useCanvasState();
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-white">
            {(() => {
              ...
              const totalAllAreas = allImageAreas.reduce(
                (sum, area) => sum + area,
                0,
              );
              ...
              const percentages = imageHistory.map((img) => {
                ...
                return totalAllAreas > 0
                  ? Math.round((imgTotalArea / totalAllAreas) * 100)
                  : 0;
              });
              ...
              const currentImageProportion =
                proportions[imageHistory.indexOf(image)];
