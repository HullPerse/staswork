...
function TextsRenderer({
  ...
  const visibleTexts = texts.filter((text) => text.visible !== false);

  return (
    <>
      {visibleTexts.map((textElement) => {
        ...
        return (
          <g key={textElement.id}>
            ...
            <text
              ...
              onClick={(e) =>
                onTextClick?.(e as unknown as React.MouseEvent, textElement.id)
              }
            >
              {textElement.text}
            </text>
...
interface HistoryTextRendererProps {
  texts: TextElement[];
}

function HistoryTextsRenderer({ texts }: HistoryTextRendererProps) {
  const visibleTexts = texts.filter((text) => text.visible !== false);

  return (
    <>
      {visibleTexts.map((textElement, index) => (
        <text
...
export const HistoryText = memo(HistoryTextsRenderer);
